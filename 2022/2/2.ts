...
const totalPointsOption1 = lines
  .map((line) => line.split(' '))
  .reduce((acc, curr) => {
...
const totalPointsOption2 = lines
  .map((line) => line.split(' '))
  .reduce((acc, curr) => {
    const opponentMove = OpponentMove.get(curr[0] as 'A' | 'B' | 'C')!;
    const desiredOutcome = DesiredOutcome.get(curr[1] as 'X' | 'Y' | 'Z')!;
    const move = [...(Outcomes.get(opponentMove)?.entries() ?? [])].find(
      ([move, outcome]) => outcome === desiredOutcome
    )?.[0];

    return (
      acc + PointsPerOutcome.get(desiredOutcome)! + PointsPerMove.get(move!)!
    );
  }, 0);
