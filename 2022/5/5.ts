...
const numberOfColumns = stackLines
  .pop()
  ?.split('   ')
  .map((val) => val.trim()).length!;
  ...
  .map((line) =>
    line
      ...
      .reduce((acc, curr, currIdx) => {
        ...
        return accIdx === acc.length
          ? [...acc, curr]
          : [...acc.slice(0, -1), `${acc[accIdx]}${curr}`];
      }, [] as string[])
      .map((el) => el.trim())
  )
  .reduce(
    (acc, curr) => {
      curr.forEach((crate, columnIdx) => {
        if (crate !== '') {
...
moveLines
  ...
  .forEach((move) => {
    ...
    for (const moveIdx of getNumberRange(parseInt(move[1]))) {
      reorderedStacks9000[parseInt(move[5]) - 1].push(
        reorderedStacks9000[parseInt(move[3]) - 1].pop()!
      );
...
const stackTops9000 = reorderedStacks9000
  .map((stack) => stack[stack.length - 1][1])
  .join('');
const stackTops9001 = reorderedStacks9001
  .map((stack) => stack[stack.length - 1][1])
  .join('');
